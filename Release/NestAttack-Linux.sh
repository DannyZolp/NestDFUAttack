...
if [ ! -f "Nest/u-boot.bin" ] || [ ! -f "Nest/x-load.bin" ] || [ ! -f "Nest/uImage" ]; then
    ...
    echo "[I] - After entering your password, you will need to hold down the screen on your nest for about 10-15 seconds (to enter DFU mode)." 1>&3
        ...
        sudo Linux/omap3_usbload -f 'Nest/x-load.bin' -a 0x080100000 -f 'Nest/u-boot.bin' -a 0x080A00000 -f 'Nest/uImage' -v -j 0x080100000 | tee /dev/fd/3
        status=${PIPESTATUS[0]}
        if [ $status -ne 0 ]; then
            echo "[E] - omap3_usbload failed!" 1>&3
            exit $status
        fi
